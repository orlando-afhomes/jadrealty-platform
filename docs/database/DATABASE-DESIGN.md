...
- **Auth:** `Supabase Auth` (`email/password` for `admin@jad.local` / `user@jad.local` — passwords from `SUPABASE_SEED_*` env) replaces `MockSessionProvider` `localStorage jad:mock:session` (cross-origin `5173` vs `5174` bug). `RLS` `members` `auth.uid() = id` `vouchers memberId = auth.uid()` `SUPER_ADMIN` bypass `service_role`. Fresh-start Phase 1 roles: `admin` / `user` via `MemberRole`.
